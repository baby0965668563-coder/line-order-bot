...
function makeTaipeiDate(year, month, day, hour, minute, second = 0) {
...
function parseOrders(text) {
  ...
  let currentPrice = 0;
  let lastPrice = 0;
  let expectItemAfterPriceOnly = false;
  let priceMap = {};
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (itemSymbol) {
      ...
      expectItemAfterPriceOnly = false;
      priceMap = {};
      continue;
    }
    ...
    if (!/[+＋*＊]/.test(line)) {
      ...
      if (currentItem && (currentPrice > 0 || lastPrice > 0) && !isTrashLine(line)) {
        ...
        for (const n of names) {
          const parsedName = extractParenNote(n);
          add(currentItem, currentPrice || lastPrice, parsedName.main, 1, parsedName.note);
        }
...
function getNextCloseDate(hour, minute) {
  const nowParts = getTaipeiParts();
  let target = makeTaipeiDate(nowParts.year, nowParts.month, nowParts.day, hour, minute, 0);

  if (target.getTime() < Date.now() - 90 * 1000) {
    target = makeTaipeiDate(nowParts.year, nowParts.month, nowParts.day + 1, hour, minute, 0);
  }
